...
const profileImg2 = new URL(`./assets/${details.secondaryProfile}`, import.meta.url).href;
...
function App() {
  ...
  return (
    <section className='Vcard'>
      <div className='flip' onClick={() => setStatus(prev => !prev)}>
        <div className={`flipBall ${status ? "open" : ""}`} onClick={() => setStatus(prev => !prev)}></div>
      </div>
      {!status ?
        <div className='section-main front'>
        ...
        <div className='section-main back'>
          <div className='section-0'>
